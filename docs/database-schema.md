...
| `newcleus-users` | ユーザープロフィール（権限情報なし） | `cmsnucleus_member` |
...
| `allowedOrigins` | `L` | ○ | CORS許可オリジン (例: `["https://sample-clinic.com"]`) | — |
...
{
  "adminUsers": [
    { "userId": "abc123", "email": "user@example.com", "displayName": "田中太郎", "status": "active" },
...
## 5. newcleus-users テーブル

ユーザープロフィール。権限情報は保持せず、Cognito Custom Attributes のみで管理。

- **テーブル名**: `newcleus-users`
- **キー設計**: PK のみ

| キー | 属性名 | 型 | 値 |
|------|--------|----|----|
| PK | `userId` | `S` | Cognito の `sub`（UUID） |

### 属性

| 属性名 | 型 | 必須 | 説明 | Nucleus対応 |
|--------|-----|------|------|-------------|
| `userId` | `S` | ○ | PK。Cognito の `sub` | `mnumber` |
| `email` | `S` | ○ | メールアドレス | `memail` |
| `displayName` | `S` | — | 表示名 | `mrealname` |
| `createdAt` | `S` | ○ | アカウント作成日時（ISO 8601） | — |
| `updatedAt` | `S` | ○ | 最終更新日時（ISO 8601） | — |

※ 権限情報 (`role`, `siteIds`) は一切保持しない。Cognito Custom Attributes が唯一の権限ソース。

### アクセスパターン

| # | 操作 | アクセス方法 | 用途 |
|---|------|-------------|------|
| 1 | GetItem | PK=`{userId}` | ユーザー情報取得 |
| 2 | PutItem | PK=`{userId}` | ユーザー作成 |
| 3 | UpdateItem | PK=`{userId}` | ユーザー更新 |

### Cognito Custom Attributes（権限管理の唯一のソース）
