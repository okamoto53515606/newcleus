...
| 2 | Scan | — | 全サイト一覧（admin用。サイト数が少ないためスキャンで対応） |
| 3 | PutItem | PK=`{siteId}` | サイト作成 |
| 4 | UpdateItem | PK=`{siteId}` | サイト更新 |
| 5 | DeleteItem | PK=`{siteId}` | サイト削除 |
...
#### GSI1: `content-types-by-createdAt`

siteId 配下のコンテンツタイプを一覧取得する（管理画面用）。

| キー | 属性名 | 型 | 説明 |
|------|--------|-----|------|
| GSI1-PK | `siteId` | `S` | 親サイトのUUID |
| GSI1-SK | `createdAt` | `S` | 作成日時（ソートキー） |
...
| 3 | PutItem | PK=`{siteId}`, SK=`{ctId}` | コンテンツタイプ作成 |
| 4 | UpdateItem | PK=`{siteId}`, SK=`{ctId}` | コンテンツタイプ更新 |
| 5 | DeleteItem | PK=`{siteId}`, SK=`{ctId}` | コンテンツタイプ削除 |
...
| `tools.nextHref` | `S` | 次ページへのリンクURL（ページネーション用。次ページなしの場合は空文字） |
| `tools.prevHref` | `S` | 前ページへのリンクURL（ページネーション用。前ページなしの場合は空文字） |
...
なし（テンプレートは ctId の Query で全件取得し、アプリ側で shortname によるフィルタリングを行う）。

### アクセスパターン

| # | 操作 | アクセス方法 | 用途 |
|---|------|-------------|------|
| 1 | Query | PK=`{ctId}` | コンテンツタイプのテンプレート一覧（アプリ側で shortname フィルタ） |
| 2 | GetItem | PK=`{ctId}`, SK=`{templateId}` | テンプレート取得 |
| 3 | PutItem | PK=`{ctId}`, SK=`{templateId}` | テンプレート作成 |
| 4 | UpdateItem | PK=`{ctId}`, SK=`{templateId}` | テンプレート更新 |
| 5 | DeleteItem | PK=`{ctId}`, SK=`{templateId}` | テンプレート削除 |
